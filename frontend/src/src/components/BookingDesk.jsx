...
import { Search, RotateCcw, CheckCircle2, UserCheck, PackageCheck, AlertCircle } from 'lucide-react';
...
const BookingDesk = ({ showNotification }) => {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div className={`lg:col-span-5 bg-slate-900/80 border p-6 rounded-2xl shadow-xl backdrop-blur-md transition-all duration-300 ${
                isExistingCustomer
                ...
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xs font-bold text-slate-300 uppercase tracking-wider flex items-center gap-2">
                        <UserCheck className="h-4 w-4 text-blue-400" /> 1. Sender / Client Verification
                    </h3>
                    {isExistingCustomer && (
                        <span className="text-[10px] bg-blue-500/10 text-blue-400 border border-blue-500/30 px-2.5 py-1 rounded-full font-bold">
                            PROFILE LOADED
                        </span>
                    )}
                    {isJustRegistered && (
                ...
                <div className="space-y-2 mb-6">
                    <label className="text-slate-400 text-xs font-medium">Search Client Phone</label>
                    <div className="flex gap-2">
                        <input
                            ...
                            className="bg-slate-950/60 border border-slate-800 px-4 py-2.5 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-blue-500 flex-1 font-mono disabled:opacity-50"
                        />
                        {isExistingCustomer || isJustRegistered ? (
                            <button
                                type="button"
                                onClick={() => resetCustomerFields(false)}
                                className="bg-red-600/10 hover:bg-red-600/20 border border-red-500/20 text-red-400 px-3 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer"
                            >
                                Clear
                            </button>
                        ) : (
                            <button
                                ...
                                className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 disabled:opacity-50"
                            >
                ...
                <div className="space-y-4 pt-4 border-t border-slate-800/80">
                    <div>
                        ...
                        <input
                            ...
                            className="bg-slate-950/60 border border-slate-800 w-full px-4 py-2.5 rounded-xl text-xs text-slate-200 disabled:opacity-50 focus:outline-none focus:border-blue-500"
                        />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            className="bg-slate-950/60 border border-slate-800 w-full px-4 py-2.5 rounded-xl text-xs text-slate-200 disabled:opacity-50 focus:outline-none focus:border-blue-500 resize-none"
                        />
                    </div>
                    <div>
                        ...
                        <input
                            ...
                            className="bg-slate-950/60 border border-slate-800 w-full px-4 py-2.5 rounded-xl text-xs text-slate-200 disabled:opacity-50 focus:outline-none focus:border-blue-500 font-mono"
                        />
                    </div>
                </div>
            </div>
            ...
            <form onSubmit={handleBookingSubmit} className="lg:col-span-7 bg-slate-900/80 border border-slate-800 p-6 rounded-2xl shadow-xl backdrop-blur-md flex flex-col justify-between">
                <div>
                    <h3 className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-6 flex items-center gap-2">
                        <PackageCheck className="h-4 w-4 text-blue-400" /> 2. Destination & Package Specifications
                    </h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                ...
                                className="bg-slate-950/60 border border-slate-800 w-full px-4 py-2.5 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-blue-500"
                            />
                        </div>
                        <div>
                            ...
                            <input
                                ...
                                className="bg-slate-950/60 border border-slate-800 w-full px-4 py-2.5 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-blue-500 font-mono"
                            />
                        </div>
                ...
                <div className="mt-8 pt-4 border-t border-slate-800/80 flex justify-end gap-3">
                    <button
                        type="button"
                        onClick={resetFormAll}
                        className="px-4 py-2.5 border border-slate-800 hover:bg-slate-800 text-slate-400 hover:text-slate-200 rounded-xl text-xs font-bold uppercase transition-all cursor-pointer flex items-center gap-1.5"
                    >
                        <RotateCcw className="h-3.5 w-3.5" /> Reset Matrix
                    </button>
                    <button
                        type="submit"
                        className="px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white rounded-xl text-xs font-bold uppercase tracking-wider transition-all cursor-pointer shadow-lg shadow-blue-500/20 flex items-center gap-1.5"
                    >
                        <CheckCircle2 className="h-3.5 w-3.5" /> Book & Lock Shipment
                    </button>
                </div>
            </form>
        </div>
